...
window.filterCheckInLog = function() {
    ...
    const filtered = (window.allCheckInLogs || []).filter(e => {
    ...
    if (countEl) countEl.textContent = `Showing ${filtered.length} of ${window.allCheckInLogs?.length || 0} logs`;
};
...
function renderCheckInLog(entries) {
    ...
    if (!entries || entries.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" class="px-4 py-10 text-center text-gray-400">No check-in records yet. Once guests sign in via the kiosk, their entries will appear here.</td></tr>';
        return;
    }
